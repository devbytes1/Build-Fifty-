...
import { 
  ...
  Lock, PlusCircle, ChevronRight, Loader, Construction,
  XCircle, CheckCircle 
} from 'lucide-react';
...
const PORTFOLIO_ITEMS = [
  { id: 1, title: 'Bella Bakery', category: 'Food', image: 'bg-orange-100', desc: 'Online ordering system & brand refresh.' },
  { id: 2, title: 'FitLife Gym', category: 'Fitness', image: 'bg-blue-100', desc: 'Membership portal and class booking.' },
  { id: 3, title: 'Urban Realty', category: 'Real Estate', image: 'bg-emerald-100', desc: 'Property listing engine with map view.' },
  { id: 4, title: 'Law & Co', category: 'Service', image: 'bg-slate-200', desc: 'Professional trust-building corporate site.' },
];
...
const BrandLogo = ({ className = "w-8 h-8" }) => (
  <svg viewBox="0 0 100 100" className={className} fill="none" xmlns="http://www.w3.org/2000/svg">
    {/* 3 Bars (Dark in light mode, White in dark mode) */}
    ...
    {/* Growth Arrow (Blue) with white outline for overlap effect */}
    <path 
      d="M10 60 L35 35 L50 50 L85 15" 
      stroke="#3B82F6" 
      strokeWidth="12" 
      strokeLinecap="round" 
...
const Button = ({ children, variant = 'primary', className = '', onClick, type = 'button' }) => {
  const baseStyle = "px-6 py-3 rounded-lg font-semibold transition-all duration-200 flex items-center justify-center gap-2 active:scale-95";
  const variants = {
...
const Services = ({ setPage }) => {
  const [activeAddOnCategory, setActiveAddOnCategory] = useState('web');
...
const Portfolio = ({ setPage }) => {
  // Placeholder data for the "Work in Progress" cards
          ...
          <AnimatePresence>
            {wipItems.map((item, i) => (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, delay: i * 0.1 }} // Staggered delay
              ...
              >
                ...
                {/* Abstract Blurred Shapes (Hinting at design) */}
